...
export type CodexTransportResponse = CodexTransportSuccess | CodexTransportFailure;

export type CodexTransportRequest = {
  model: string;
...
export type CodexTransport = (request: CodexTransportRequest) => Promise<CodexTransportResponse>;
...
class TimeoutError extends Error {
  ...
  constructor(timeoutMs: number) {
    super(`codex transport timed out after ${timeoutMs}ms`);
...
function normalizeProviderErrorMessage(code: string): string {
  ...
  switch (normalizedCode) {
    ...
    case "TOO_MANY_REQUESTS":
      return "codex provider rate limit exceeded";
    case "AUTH":
    case "AUTHENTICATION":
    case "UNAUTHORIZED":
    case "FORBIDDEN":
      return "codex provider authentication failed";
    case "INVALID_REQUEST":
    case "BAD_REQUEST":
      return "codex provider rejected the request";
    case "SERVICE_UNAVAILABLE":
    case "UNAVAILABLE":
      return "codex provider service unavailable";
    case "TIMEOUT":
      return "codex provider request timed out";
    default:
      return "codex provider returned an error (details redacted)";
  }
}
...
/**
 * Creates a Codex adapter that normalizes transport responses to the shared LLM contract.
 *
 * @throws {Error} Throws when normalized response data violates the shared response schema.
 */
export function createCodexAdapter(dependencies: CodexAdapterDependencies): ProviderAdapter {
  const now = dependencies.now ?? Date.now;

  return async function callCodex(request: LlmRequest): Promise<LlmResponse> {
    const startedAt = now();
    ...
    try {
      ...
      if (error instanceof TimeoutError) {
        return normalizeFailure(
          request,
          "TIMEOUT",
          `codex request timed out after ${error.timeoutMs}ms`,
          latencyMs,
        );
      }
